...
int main() {
  ...
  a[2] = Pt(3, 3);
  for (int i = 0; i < 5; i++)
    cout << i << "," << i << ": " << inside(a,3, Pt(i, i)) << endl;
  for (int i = 0; i < 5; i++)
    cout << i << "," << i << ": " << inside_wn(a,3, Pt(i, i)) << endl;
  for (int i = 3; i < N; i++)
    a[i] = Pt(cos(i * pi / N) * 2 + 1, sin(i * pi / N) + 3);
  a[N / 2] = Pt(1, 4);
  for (int i = -1; i < 10; i++) {
    Pt p = Pt(3 - i / 2.5, -1 + i);
    cout << p.x << "," << p.y << ": " << inside(a,N, p) << endl;
  }
  for (int i = -1; i < 10; i++) {
    Pt p = Pt(3 - i / 2.5, -1 + i);
    cout << p.x << "," << p.y << ": " << inside_wn(a,N, p) << endl;
  }
  ...
  for( int y=9; y>=-1; y-- ) {
    ...
    for( int x=-1; x<=9; x++ )
      cout << inside(poly.begin(),poly.size(),point<int>(x,y));
    cout << "   ";
