...
public class Hex extends JPanel {
    ...
    static final Shape HEX = hexShape();
    static Shape hexShape() {
    ...
    static void hexPaint(Graphics2D g, int r, int c) {
    ...
    static void hexScale(Graphics2D g, int r, int c, double w, double h) {
	...
	Dimension d = getSize();
	hexScale(g, N, M, d.getWidth(), d.getHeight());
	hexPaint(g, N, M);
    }
    ...
    static class HexPrint implements Printable {
    ...
    static void hexSize(String[] args, int i) {
	if (args.length > i)
	...
	if (args.length > 0 && args[0].equals("print")) {
		...
		hexSize(args, 1);
	    try {
		System.out.println("Printing " + N + "x" + M + " hex board");
		job.print();
	...
	System.out.println("usage: " + 
			   "java Hex [print [dialog]] " +
			   "[N [M [[no]label [rect]]]]");
	hexSize(args, 0);
	JFrame frame = new JFrame("Hex " + N + "x" + M);
	frame.addWindowListener(new Window());
	JPanel panel = new Hex();
